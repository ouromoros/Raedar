...
本项目包含两个Module，`app`是主要的UI及相关的逻辑的代码，`backend`中包含网络、数据库交互等的代码。
...
- 在RxJava的使用中若处理不当则可能在`Activity`已经退出的情况下进程依然运行，造成内存泄露。本软件使用了`rxlifecycle`来处理这种情况，避免了内存泄露的情况发生。
